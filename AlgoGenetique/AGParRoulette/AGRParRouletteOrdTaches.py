...
def selection_par_roulette(population, couts):
    total_fitness = sum(1/c for c in couts)
    probabilities = [(1/c) / total_fitness for c in couts]
    return population[random.choices(range(len(population)), weights=probabilities)[0]]

def crossover_par_roulette(population, couts):
    parent1 = selection_par_roulette(population, couts)
    parent2 = selection_par_roulette(population, couts)
    taille = len(parent1)
...
def algorithme_genetique(cout_matrix, taille_population, taux_mutation, max_generations):
    ...
    for generation in range(max_generations):
        ...
        for _ in range(taille_population):
            enfant = crossover_par_roulette(population, couts)
