...
def selection_par_roulette(population, distances):
...
def crossover_par_roulette(population, distances):
    parent1 = selection_par_roulette(population, distances)
    parent2 = selection_par_roulette(population, distances)
    taille = len(parent1)
...
def algorithme_genetique(distance_matrix, taille_population, taux_mutation, max_generations):
    ...
    for generation in range(max_generations):
        ...
        for _ in range(taille_population):
            enfant = crossover_par_roulette(population, distances)
